...
func (mj *TimePeriod) MarshalJSON() ([]byte, error) {
	var buf fflib.Buffer
	if mj == nil {
		buf.WriteString("null")
		return buf.Bytes(), nil
...
func (mj *TimePeriod) MarshalJSONBuf(buf fflib.EncodingBuffer) error {
	if mj == nil {
		buf.WriteString("null")
		return nil
	}
	...
	if mj.Period != 0 {
		buf.WriteString(`"period":`)
		fflib.FormatBits2(buf, uint64(mj.Period), 10, mj.Period < 0)
		buf.WriteByte(',')
	}
	if true {
		buf.WriteString(`"period_date":`)
...
const (
	ffj_t_TimePeriodbase = iota
	ffj_t_TimePeriodno_such_key

	ffj_t_TimePeriod_Period

	ffj_t_TimePeriod_PeriodDate
)

var ffj_key_TimePeriod_Period = []byte("period")

var ffj_key_TimePeriod_PeriodDate = []byte("period_date")

func (uj *TimePeriod) UnmarshalJSON(input []byte) error {
	fs := fflib.NewFFLexer(input)
	return uj.UnmarshalJSONFFLexer(fs, fflib.FFParse_map_start)
}

func (uj *TimePeriod) UnmarshalJSONFFLexer(fs *fflib.FFLexer, state fflib.FFParseState) error {
	var err error = nil
	currentKey := ffj_t_TimePeriodbase
	_ = currentKey
	tok := fflib.FFTok_init
	wantedTok := fflib.FFTok_init
...
mainparse:
	for {
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				// "" case. hrm.
				currentKey = ffj_t_TimePeriodno_such_key
				state = fflib.FFParse_want_colon
				goto mainparse
			} else {
				switch kn[0] {
				...
				case 'p':
					...
					if bytes.Equal(ffj_key_TimePeriod_Period, kn) {
						currentKey = ffj_t_TimePeriod_Period
						state = fflib.FFParse_want_colon
						goto mainparse

					} else if bytes.Equal(ffj_key_TimePeriod_PeriodDate, kn) {
						currentKey = ffj_t_TimePeriod_PeriodDate
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				if fflib.AsciiEqualFold(ffj_key_TimePeriod_PeriodDate, kn) {
					currentKey = ffj_t_TimePeriod_PeriodDate
					state = fflib.FFParse_want_colon
					goto mainparse
				}

				if fflib.SimpleLetterEqualFold(ffj_key_TimePeriod_Period, kn) {
					currentKey = ffj_t_TimePeriod_Period
					state = fflib.FFParse_want_colon
					goto mainparse
				}
		...
		case fflib.FFParse_want_value:
			...
			if tok == fflib.FFTok_left_brace || tok == fflib.FFTok_left_bracket || tok == fflib.FFTok_integer || tok == fflib.FFTok_double || tok == fflib.FFTok_string || tok == fflib.FFTok_bool || tok == fflib.FFTok_null {
				...
				case ffj_t_TimePeriod_Period:
					goto handle_Period

				case ffj_t_TimePeriod_PeriodDate:
					goto handle_PeriodDate

				case ffj_t_TimePeriodno_such_key:
					err = fs.SkipField(tok)
					if err != nil {
						return fs.WrapErr(err)
					}
...
handle_Period:
	...
	/* handler: uj.Period type=common.Period kind=int quoted=false*/

	{
		if tok != fflib.FFTok_integer && tok != fflib.FFTok_null {
			return fs.WrapErr(fmt.Errorf("cannot unmarshal %s into Go value for Period", tok))
		}
	}
